...
from trainers_profile.trainers_certifications.models import TrainerCertification
from trainers_profile.trainers_documents.models import TrainerDocument
# Code add with me
from trainers_profile.models import TrainersPersonalDetails as TrainerProfileEntry
#======================
...
def safe_int(val):
    """
    Cache se aayi values kabhi kabhi string hoti hain.
    Ye helper safely integer mein convert karta hai.
    Agar convert na ho sake (None, '', 'true', etc.) to None return karta hai.
    """
    if val is None:
        return None
    try:
...
class TrainerBankDetailsViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        with transaction.atomic():
            ...
            for cert in temp_certs:
                TrainerCertification.objects.create(
                    ...
                    # ✅ FIX: safe_int() use karo — cache se string aa sakti hai
                    year_of_graduation=safe_int(cert.get("year_of_graduation")),
                    work_experience=safe_int(cert.get("work_experience")),
        ...
        # Code add with me
        # Admin approval tracker mein record banao (trainers_profile)
        # Ye step isliye zaroori hai taaki Admin pending list mein trainer dekh sake
        TrainerProfileEntry.objects.get_or_create(
            phone=temp_personal["contact_number"],
            defaults={
                "full_name": temp_personal.get("full_name", ""),
                "email": temp_personal.get("email", ""),
                "password": "",   # Real auth authapp.User mein hoti hai
                "status": "pending"
            }
        )
        #======================
